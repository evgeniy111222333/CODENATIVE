...
def test_repo_graph_query_returns_bias_hits_and_copy_support() -> None:
    ...
    assert memory.candidate_index is not None
    assert "app/core.py" in memory.candidate_index.nodes_by_file
    assert "function" in memory.candidate_index.nodes_by_kind
    assert "GRAPH_SHARED_NAME" in memory.candidate_index.nodes_by_copy_term
    with torch.no_grad():
    ...
    assert result.total_node_count == len(index.nodes)
    assert result.candidate_pool_size == result.candidate_count
    assert result.candidate_count < result.total_node_count
...
def test_repo_graph_budget_keeps_exact_symbol_target() -> None:
    config = HTMCodeNativeConfig.from_yaml(Path("configs/phase_a.yaml"))
    indexer = RepositoryGraphIndexer(
        key_dim=config.model.graph_key_dim,
        value_dim=config.model.graph_value_dim,
    ...
    assert result.candidate_count < len(index.nodes)
    assert expected_target_ids.intersection(result.candidate_node_ids)
    assert result.target_node_id in expected_target_ids


def test_repo_graph_edit_query_keeps_diagnostic_test_and_symbol_candidates() -> None:
    config = HTMCodeNativeConfig.from_yaml(Path("configs/phase_a.yaml"))
    indexer = RepositoryGraphIndexer(
        key_dim=config.model.graph_key_dim,
        value_dim=config.model.graph_value_dim,
    ...
    result = memory.query(
        ...
        context=RepoGraphQueryContext(
            ...
            target_symbol_name="GRAPH_SHARED_NAME",
            target_token_value="GRAPH_SHARED_NAME",
            target_copy_value='"shared_graph_token"',
        ),
        vocabulary_snapshot=VocabularyRegistry(config.model.vocabulary_size).snapshot(),
    )

    assert result.candidate_count < len(index.nodes)
    assert any(kind in {"symbol", "function", "class"} for kind in result.candidate_kinds)
    assert "diagnostic" in result.candidate_kinds
    assert "test" in result.candidate_kinds


def test_repo_graph_query_falls_back_when_candidate_pool_is_empty() -> None:
    config = HTMCodeNativeConfig.from_yaml(Path("configs/phase_a.yaml"))
    indexer = RepositoryGraphIndexer(
        key_dim=config.model.graph_key_dim,
        value_dim=config.model.graph_value_dim,
    ...
    assert result.candidate_count == len(index.nodes)
    assert result.candidate_pool_size == len(index.nodes)
    assert result.pruned_node_count == 0
    assert result.prune_rate == 0.0
